...
div
  ...
  //-     i.el-icon-close(@click="closeWebview")
  //-   span 訂單明細
  //-   div
  .order__container
...
<script>
// import { getOrderSingle } from '@/api'
export default {
  name: 'OrderSingle',
  data () {
    return {
      list: [],
      activeName: Number(this.$route.params.order_id)
    }
  },
  created () {
    document.title = '訂單明細'
  ...
  methods: {
    ...
    fetchData () {
      ...
      // getOrderSingle({'order_id':this.$route.params.order_id}).then(res => {
