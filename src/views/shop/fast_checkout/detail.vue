<template lang="pug">
  div
    ...
    el-form(ref='form' :rules="rules" label-position="top" :model='form' label-width='80px' class="detail__form")
      ...
      el-form-item(label='收件人工號' prop="to_user_id" )
        el-input(v-model='form.to_user_id' placeholder="")

      el-form-item(label='收件人姓名' prop="to_user_name" )
        el-input(v-model='form.to_user_name' placeholder="")

      el-form-item(label='給收件人的一段話(100字以內)' prop="to_user_text" )
        el-input(v-model='form.to_user_text' type="textarea" :autosize="{ minRows: 3}")
      ...
      el-dialog(:visible.sync='dialog', width='85%')
        ...
        span.dialogBody 訂單金額：
          span.ntd NTD $ {{(totalPrice || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,')}}             
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        to_user_id: [{ required: true, message: '請輸入收件人', trigger: 'blur' }],
        to_user_name: [{ required: true, message: '請輸入收件人姓名', trigger: 'blur' }],
        to_user_text: [{ required: true, message: '請輸入給收件人的話', trigger: 'blur' }],
      },
      dialog: false,
      disabled: true,
    }
  ...
  methods: {
    ...
    submitConfirmAmount(){
      ...
      this.$emit('keyInDetail', this.form)
      this.$emit('setStep', 1)
      this.$router.push({ name: 'fast_checkout_payment' })
    },
    myTouchMove(evt){
  ...
  computed: {
    ...
    priceFormat: function(){
        ...
        return ( this.totalPrice || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,');
    },
    btnStyleClass () {
