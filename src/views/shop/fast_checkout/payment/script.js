...
// import { createActivityOrder } from '@/api/index'
...
export default {
  ...
  computed: {
    ...
    priceFormat: function () {
      // this.totalPrice = this.totalPrice-this.nowInputDiscount
  ...
  methods: {
    tappay () {
      ...
      let appKey =
        'app_lVKcbm5BkAgfJjUDPQpPTT2v7FISvWWvaKdunCsXV5GtwhTGKGzqcKRLHeEY'
      // 測試
      // let serverType = 'sandbox'
      // let appKey = 'app_lVKcbm5BkAgfJjUDPQpPTT2v7FISvWWvaKdunCsXV5GtwhTGKGzqcKRLHeEY'
      TPDirect.setupSDK(13118, `${appKey}`, `${serverType}`)
      // set style
    ...
    sendOrder (result) {
      ...
      // createActivityOrder(createOrderParams).then(this.responseHandler)
    },
    responseHandler (res) {
      this.loading.close()
      // code === 0 訂單成立
      ...
      switch (res.data.code) {
        case 0:
          // res.data.order_id
          // trackPurchase(res.data.order_id, this.totalPrice, this.items)
          // this.resetCart()
          this.openAlert(
            '已經收到您的費用，商品即將寄出，有任何問題請聯絡：service@tapbee.cc',
            '付款成功'
          )
          break
        case 1:
    ...
    openAlert (content, title) {
      this.$alert(content, title, {
        ...
        callback: action => {
          ...
          if (title === '付款成功') {
            this.$router.push({ name: 'fast_checkout_paySuccess' })
          } else {
